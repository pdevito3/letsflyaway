...
public static class PatchRecipe
{
    ...
    public class Handler : IRequestHandler<PatchRecipeCommand, bool>
    {
        ...
        public async Task<bool> Handle(PatchRecipeCommand request, CancellationToken cancellationToken)
        {
            ...
            var recipeToPatch = _mapper.Map<RecipeForUpdateDto>(recipeToUpdate); // map the recipe we got from the database to an updatable recipe model
            request.PatchDoc.ApplyTo(recipeToPatch); // apply patchdoc updates to the updatable recipe
